...
# if there's any apt lock failure, apply this manual fix, then re-run
# https://itsfoss.com/could-not-get-lock-error/
# $ ps aux | grep -i apt
# $ kill -9 all ref processes

echo '--------------------------------'
apt install net-tools
ifconfig -a
echo '--------------------------------'
eth0mac=$(ifconfig -a | grep eth0 -C 2 | egrep -o '.{2}:.{2}:.{2}:.{2}:.{2}:.{2}')
eth1mac=$(ifconfig -a | grep eth1 -C 2 | egrep -o '.{2}:.{2}:.{2}:.{2}:.{2}:.{2}')
while [[ -z ${eth1mac} ]]; do
